...
$Page->showMessage();
?>
<form name="fuseredit" id="fuseredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->Phone->Visible) { // Phone ?>
    <div id="r_Phone" class="form-group row">
        <label id="elh_user_Phone" for="x_Phone" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Phone->caption() ?><?= $Page->Phone->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Phone->cellAttributes() ?>>
<span id="el_user_Phone">
<input type="<?= $Page->Phone->getInputTextType() ?>" data-table="user" data-field="x_Phone" name="x_Phone" id="x_Phone" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->Phone->getPlaceHolder()) ?>" value="<?= $Page->Phone->EditValue ?>"<?= $Page->Phone->editAttributes() ?> aria-describedby="x_Phone_help">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->Enable->isInvalidClass() ?>" data-table="user" data-field="x_Enable" name="x_Enable[]" id="x_Enable_729344" value="1"<?= ConvertToBool($Page->Enable->CurrentValue) ? " checked" : "" ?><?= $Page->Enable->editAttributes() ?> aria-describedby="x_Enable_help">
    <label class="custom-control-label" for="x_Enable_729344"></label>
</div>
