...
foreach ($nodes = $xpath->query("//section[@id='slider']") as $node) {
	$node->setAttribute('class', trim(str_replace('include-topbar', '', $node->getAttribute('class'))));
	$node->setAttribute('class', trim(str_replace('min-vh-60', 'min-vh-25', $node->getAttribute('class'))));
	$node->setAttribute('class', trim(str_replace('min-vh-md-100', 'min-vh-md-25', $node->getAttribute('class'))));
}
