...
if (pathinfo($_SERVER['SCRIPT_NAME'], PATHINFO_EXTENSION) == 'html') {
	...
	if (empty($content)) {
		...
		if (pathinfo($_SERVER['SCRIPT_NAME'], PATHINFO_BASENAME) == 'intro.html') {
			...
			if (($node = $xpath->query($selector = '/html/head')) AND !empty($node->length) AND $node = $node->item(0)) {
				$link_node = $dom->createElement('link', '');
				$link_node->setAttribute('rel', 'shortcut icon');
				$link_node->setAttribute('type', 'image/x-icon');
				$link_node->setAttribute('href', 'admin/favicon.ico');
					...
					if (!empty($elements->length)) {
						...
						if (!empty($datas)) {
							foreach ($datas as $key => $data) {
								...
								if (($nodes = $xpath->query($selector = './/h2/a', $parent->item(0))) AND !empty($nodes->length))
									$nodes->item(0)->nodeValue = $data['Title'];
								...
								if (!empty($data['Intro'])) {
									...
									foreach ($temp->firstChild->childNodes as $child) {
										if ($child->nodeName == "#text") {
											$node = $dom->createTextNode($child->nodeValue);
										} else {
											$node = $dom->createElement($child->nodeName, $child->nodeValue);
										}
										$newnode = $html_element->item(0)->appendChild($node);
									}
								} else {
								...
								if (Session(SESSION_STATUS) == 'login') {
									foreach ($xpath->query('.//a', $elements) as $element) {
										$element->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/BlogEdit/{$data['Blog_ID']}'; return false; }");
									}
								}
								if ($key % 3 == 2) {
							...
							foreach ($datas as $key => $value) {
								...
								if ($key % 3 == 2)
									$parent->item(0)->appendChild($node);
							}
						}
					}
				}
					...
					if (!empty($prototype->length)) {
						...
						if (!empty($datas)) {
							foreach ($datas as $key => $data) {
								...
								if (($nodes = $xpath->query($selector = './/h2/a|.//h3/a', $parent->item(0))) AND !empty($nodes->length)) {
									$nodes->item(0)->nodeValue = $data['Title'];
								}
								...
								if (($nodes = $xpath->query('.//*[contains(@class,"entry-meta")]//li/text()', $prototype)) AND !empty($nodes->length)) {
									$nodes->item(0)->nodeValue = $data['Blog_Date'];
								}
			...
			if ($template = "Polo - Blog Detail") {
				...
				if (($parent = $xpath->query($selector = '//*[@id="blog" and @class="single-post"][div[contains(@class, "post-item")]]')) AND !empty($parent->length)) {
					...
					if (!empty($data)) {
						...
						if (($nodes = $xpath->query($selector = './/h2', $parent->item(0))) AND !empty($nodes->length))
							$nodes->item(0)->nodeValue = $data['Title'];
						$xpath->query('.//img', $elements)->item(0)->setAttribute("src", "upload/{$data['Image']}");
						$xpath->query('.//span[@class="post-meta-date"]/text()', $elements)->item(0)->nodeValue = $data['Blog_Date'];
						foreach ($xpath->query('.//span[@class="post-meta-comments"]', $elements) as $element)
						...
						if (!empty($datas)) {
							foreach ($datas as $key => $data) {
								// $data['Slug'] = slugify(mb_substr($data['Name'], 0, 80));
									...
									// $nodes->item(0)->nodeValue = $data['Name'];
								...
								if (Session(SESSION_STATUS) == 'login')
									foreach ($xpath->query('.//a', $prototype) as $element)
										$element->setAttribute("href", "admin/ProductEdit/{$_GET['Product_ID']}");
								$children[$key] = $prototype->cloneNode(true);
							}
					...
					if (($parent = $xpath->query($selector = "//div[@id='related-portfolio']")) AND !empty($parent->length)) {
						...
						if (!empty($datas = ExecuteRows("SELECT *, Product_ID AS ID FROM product WHERE Category_ID = 1 AND Product_ID != {$_GET['Product_ID']} ORDER BY Priority LIMIT 10", 2))) {
							foreach ($datas as $key => $data) {
								...
								$data['Slug'] = slugify(mb_substr($data['Name'], 0, 80));
								// fb($data['Slug']);
								if (($nodes = $xpath->query($selector = './/h3/a', $prototype)) AND !empty($nodes->length)) {
									$nodes->item(0)->nodeValue = $data['Name'];
								}
								if (($nodes = $xpath->query($selector = './/span', $prototype)) AND !empty($nodes->length)) {
									$nodes->item(0)->nodeValue = $data['Intro'];
								}
								if (($nodes = $xpath->query($selector = './/img', $prototype)) AND !empty($nodes->length) AND !empty($data['Image']) AND file_exists("upload/{$data['Image']}") AND exif_imagetype("upload/{$data['Image']}") !== false AND filter_var("upload/{$data['Image']}", FILTER_VALIDATE_URL) === false) {
									$nodes->item(0)->setAttribute("src", "upload/{$data['Image']}");
								}
								foreach (($nodes = $xpath->query($selector = './/a', $prototype)) as $node) {
									$node->setAttribute("href", "product-{$data['ID']}-{$data['Slug']}");
								}
								if (Session(SESSION_STATUS) == 'login') {
									foreach (($nodes = $xpath->query($selector = './/a', $prototype)) as $node) {
										$node->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/ProductEdit/{$data['ID']}'; return false; }");
									}
								}
								$children[$key] = $prototype->cloneNode(true);
							}
				...
				if (($parent = $xpath->query($selector = '//*[@id="portfolio"][article[contains(@class,"portfolio-item")]]')) AND !empty($parent->length)) {
					...
					if (!empty($datas)) {
						foreach ($datas as $key => $data) {
							...
							if (Session(SESSION_STATUS) == 'login')
								foreach ($xpath->query('.//a', $prototype) as $element)
									$element->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/{$data['Table_Name']}Edit/{$data['ID']}'; return false; }");
				...
				if (!empty($elements->length)) {
					...
					if (!empty($categories)) {
						foreach ($categories as $key => $value) {
							...
							if (Session(SESSION_STATUS) == 'login') {
								$elements->item(0)->parentNode->childNodes->item(1)->firstChild->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/CategoryEdit/{$value['Category_ID']}'; return false; }");
					...
					if (!empty($products)) {
						foreach ($products as $key => $value) {
							...
							if (!empty($value['Image']))
								$elements->item(0)->parentNode->parentNode->childNodes->item(0)->childNodes->item(1)->setAttribute("src", "upload/{$value['Image']}");
							if (Session(SESSION_STATUS) == 'login') {
								$elements->item(0)->parentNode->childNodes->item(1)->firstChild->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/ProductEdit/{$value['Product_ID']}'; return false; }");
								$elements->item(0)->parentNode->childNodes->item(1)->firstChild->setAttribute("style", "cursor: pointer;" . $elements->item(0)->parentNode->childNodes->item(1)->firstChild->getAttribute("style"));
							}
							$children[$key] = $elements->item(0)->parentNode->parentNode->cloneNode(true);
						}
				...
				if (!empty($elements->length)) {
					...
					if (!empty($products)) {
						foreach ($products as $key => $value) {
							...
							if (($node = $xpath->query($selector = '//article/div/span/a', $elements->item(0))) AND !empty($node->length) AND $node = $node->item(0)) {
								$node->nodeValue = $value['Intro'];
								$node->setAttribute("href", "product-{$value['Product_ID']}-{$value['Slug']}");
							}
							// $xpath->query('//article/div/span/a', $elements->item(0))->item(0)->setAttribute("href", "product-{$value['Product_ID']}-{$value['Slug']}");
							if (Session(SESSION_STATUS) == 'login') {
								$xpath->query('//article/div/a', $elements->item(0))->item(0)->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/ProductEdit/{$value['Product_ID']}'; return false; }");
								...
								$xpath->query('//article/div/h3/a', $elements->item(0))->item(0)->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/ProductEdit/{$value['Product_ID']}'; return false; }");
								...
								if (($node = $xpath->query($selector = '//article/div/span/a', $elements->item(0))) AND !empty($node->length) AND $node = $node->item(0)) {
									$node->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/ProductEdit/{$value['Product_ID']}'; return false; }");
									$node->setAttribute("style", "cursor: pointer;" . $xpath->query('//article/div/span/a', $elements->item(0))->item(0)->getAttribute("style"));
								}
								// $xpath->query('//article/div/span/a', $elements->item(0))->item(0)->setAttribute("onclick", "if (confirm('Do you want to edit this?')) { window.location.href = 'admin/ProductEdit/{$value['Product_ID']}'; return false; }");
			...
			if ($template = "Canvas - Blog Detail") {
				...
				if (($parent = $xpath->query($selector = '//*[@class="news-single-post"]')) AND !empty($parent->length)) {
					...
					if (!empty($data)) {
						...
						if (($nodes = $xpath->query($selector = './/h1', $parent->item(0))) AND !empty($nodes->length))
							$nodes->item(0)->nodeValue = $data['Title'];
						if (($nodes = $xpath->query($selector = './/h2', $parent->item(0))) AND !empty($nodes->length))
							$nodes->item(0)->nodeValue = $data['Title'];
...
//div[not(descendant::div) and not(descendant::i) and not(descendant::button) and not(descendant::input) and not(descendant::textarea) and not(descendant::select) and not(descendant::iframe) and not(not(*))]
			...
			if (!empty($elements->length)) {
				foreach ($elements as $key => $element) {
					if (Session(SESSION_STATUS) == 'login')
						...
						$elements->item($key)->setAttribute("href", "/admin");
				}
			}
			...
			if ('Edit Primitive Elements by Back-End') {
				...
				$elements = $xpath->query($selector = "
					...
					//a[normalize-space(text())!='' and @href!='/admin' and not(ancestor::*[@data-inplaze or (@id='blog' and @data-item='post-item')])]|
					...
					foreach ($elements as $key => $element) {
						...
						if ($pattern == "#text" OR $pattern == "#text,br,#text" OR $pattern == "i,#text") {
							...
							if (Session(SESSION_STATUS) == 'login') {
								...
								$elements->item($key)->setAttribute("onclick", "if (window.event.target == this && {$is_link}) { window.location.href = 'admin/TextList?cmd=search&x_Name={$id}'; return false; }");
							}
						} else {
				...
				if (!empty($elements->length)) {
					foreach ($elements as $key => $element) {
						...
						if ($pattern == "#text") {
							if (Session(SESSION_STATUS) == 'login') {
								$elements->item($key)->setAttribute("onclick", "window.location.href = 'admin/ContentsList?cmd=search&x_Name={$id}'; return false;");
								$elements->item($key)->setAttribute("style", "cursor: pointer;" . $elements->item($key)->getAttribute("style"));
							}
					...
					foreach ($elements as $key => $element) {
						...
						if (Session(SESSION_STATUS) == 'login') {
							...
							if (
								...
								$elements->item($key)->parentNode->nextSibling->nextSibling->setAttribute("onclick", "if (window.event.target == this && {$is_link}) { window.location.href = 'admin/ImageList?cmd=search&x_Name={$id}'; return false; }");
							} else if (
								...
								$elements->item($key)->nextSibling->setAttribute("onclick", "if (window.event.target == this && {$is_link}) { window.location.href = 'admin/ImageList?cmd=search&x_Name={$id}'; return false; }");
							} else {
								$elements->item($key)->setAttribute("style", "cursor: pointer;" . $elements->item($key)->getAttribute("style"));
								$elements->item($key)->setAttribute("onclick", "if (window.event.target == this && {$is_link}) { window.location.href = 'admin/ImageList?cmd=search&x_Name={$id}'; return false; }");
							}
						}
					}
				}
				...
				if (!empty($elements->length)) {
					foreach ($elements as $key => $element) {
						...
						// preg_match_all('/(?<=url\()(\'|")?(?<image>.*?)(?=\1|\))/i',$elements->item($key)->getAttribute("style"),$matches);
						if (!empty($matches['image'][0])) {
							if (!empty($matches['image'][0]) AND file_exists("upload/{$matches['image'][0]}") AND exif_imagetype("upload/{$matches['image'][0]}") !== false AND filter_var("upload/{$matches['image'][0]}", FILTER_VALIDATE_URL) === false) {
							...
							// fb($selector);
							// fb($id);
							if (!empty($value = ExecuteScalar("SELECT Value FROM image WHERE Enable = 1 AND Name = '{$id}'"))) {
								$elements->item($key)->setAttribute("style", str_replace($matches['image'][0], "upload/{$value}", $elements->item($key)->getAttribute("style")));
							}
							if (Session(SESSION_STATUS) == 'login') {
								// if (($nodes = $xpath->query($selector = './/div[contains(@class, "container")]', $elements->item($key)->parentNode)) AND !empty($nodes->length)) {
									// $nodes->item(0)->setAttribute("style", "cursor: pointer;" . $nodes->item(0)->getAttribute("style"));
									// $nodes->item(0)->setAttribute("onclick", "window.location.href = 'admin/ImageList?cmd=search&x_Name={$id}'; return false;");
								// } else {
									$elements->item($key)->setAttribute("style", "cursor: pointer;" . $elements->item($key)->getAttribute("style"));
									$elements->item($key)->setAttribute("onclick", "if (window.event.target == this || window.event.target.parentElement == this) { window.location.href = 'admin/ImageList?cmd=search&x_Name={$id}'; return false; }");
									$elements->item($key)->setAttribute("title", "type=\"{$image_info["mime"]}\" {$image_info[3]}");
								// }
								// fb($elements->item($key)->parentNode->nodeName, 'nodeName');
								// fb($elements->item($key)->parentNode->getAttribute('class'), 'class');
								// fb($elements->item($key)->parentNode->childNodes->item(1)->childNodes->item(1)->nodeName);
								// fb($elements->item($key)->parentNode->childNodes->item(1)->childNodes->item(1)->getAttribute('class'));
								// fb(($target = $elements->item($key)->parentNode->childNodes->item(1)->childNodes->item(1)) != NULL, 'target');
								// http://canvas.in-plaze.local/index.html
								// die(!empty($elements->item($key)));
								// die(!empty($elements->item($key)->parentNode));
								// die(!empty($elements->item($key)->parentNode->childNodes));
								// die(!empty($elements->item($key)->parentNode->childNodes->item(1)));
								// die();
								// die(!empty($elements->item($key)->parentNode->childNodes->item(1)->childNodes->item(1)));
								if (!empty($elements->item($key)->parentNode->childNodes->item(1)->childNodes) AND ($target = $elements->item($key)->parentNode->childNodes->item(1)->childNodes->item(1)) != NULL) {
									if (strpos($elements->item($key)->parentNode->getAttribute('class'), 'swiper-slide') !== false) {
										// fb($selector, 'selector');
										// fb($id, 'id');
										// fb($nodes->length, 'nodes');
										// fb($nodes->item(0)->nodeName, 'nodes');
										// fb($elements->item($key)->parentNode->nodeName, 'parentNode');
										// fb(, 'parentNode');
										// fb(strpos($elements->item($key)->parentNode->getAttribute('class'), 'swiper-slide') !== false, 'parentNode');
										// $nodes->item(0)->setAttribute("style", "cursor: pointer;" . $nodes->item(0)->getAttribute("style"));
										// fb($elements->item($key)->parentNode->childNodes->item(1)->childNodes->item(1)->nodeName);
										// fb($elements->item($key)->parentNode->childNodes->item(1)->childNodes->item(1)->childNodes->length);
										$i_node = $dom->createElement("i");
										$i_node->setAttribute("class", "icon-picture");
										$a_node = $dom->createElement("a");
										$a_node->appendChild($i_node);
										$a_node->setAttribute("href", "admin/ImageList?cmd=search&x_Name={$id}");
										$a_node->setAttribute("title", "type=\"{$image_info["mime"]}\" {$image_info[3]}");
										// if (exif_imagetype($elements->item($key)->getAttribute("src")) !== false AND filter_var($elements->item($key)->getAttribute("src"), FILTER_VALIDATE_URL) === false) {
											// $image_info = getimagesize(urldecode($elements->item($key)->getAttribute("src")));
				...
				if (!empty($elements->length)) {
					foreach ($elements as $key => $element) {
						...
						if (Session(SESSION_STATUS) == 'login') {
							$elements->item($key)->setAttribute("onclick", "if (window.event.target == this && confirm('Do you want to edit this?')) { window.location.href = 'admin/TextList?cmd=search&x_Name={$id}'; return false; }");
						}
					}
				}
				...
				if (!empty($elements->length)) {
					foreach ($elements as $key => $element) {
						...
						if (Session(SESSION_STATUS) == 'login') {
							...
							$a_node = $dom->createElement("a");
							$a_node->appendChild($i_node);
							$a_node->setAttribute("href", "admin/TextList?cmd=search&x_Name={$id}");
