...
class Image extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // Image_ID
        $this->Image_ID = new DbField('image', 'image', 'x_Image_ID', 'Image_ID', '`Image_ID`', '`Image_ID`', 3, 11, -1, false, '`Image_ID`', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->Image_ID->IsAutoIncrement = true; // Autoincrement field
        $this->Image_ID->IsPrimaryKey = true; // Primary key field
        $this->Image_ID->Sortable = true; // Allow sort
        $this->Image_ID->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Image_ID->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->Image_ID->Param, "CustomMsg");
        $this->Fields['Image_ID'] = &$this->Image_ID;

        // Name
        $this->Name = new DbField('image', 'image', 'x_Name', 'Name', '`Name`', '`Name`', 200, 255, -1, false, '`Name`', false, false, false, 'FORMATTED TEXT', 'HIDDEN');
        $this->Name->Sortable = true; // Allow sort
        $this->Name->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->Name->Param, "CustomMsg");
        $this->Fields['Name'] = &$this->Name;

        // Value
        $this->Value = new DbField('image', 'image', 'x_Value', 'Value', '`Value`', '`Value`', 201, 65535, -1, true, '`Value`', false, false, false, 'IMAGE', 'FILE');
        ...
        // Enable
        $this->Enable = new DbField('image', 'image', 'x_Enable', 'Enable', '`Enable`', '`Enable`', 202, 1, -1, false, '`Enable`', false, false, false, 'FORMATTED TEXT', 'CHECKBOX');
        $this->Enable->Sortable = false; // Allow sort
        $this->Enable->DataType = DATATYPE_BOOLEAN;
        switch ($CurrentLanguage) {
            case "en":
                $this->Enable->Lookup = new Lookup('Enable', 'image', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
            case "th":
                $this->Enable->Lookup = new Lookup('Enable', 'image', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        // Created
        $this->Created = new DbField('image', 'image', 'x_Created', 'Created', '`Created`', CastDateFieldForLike("`Created`", 0, "DB"), 135, 19, 0, false, '`Created`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // Modified
        $this->Modified = new DbField('image', 'image', 'x_Modified', 'Modified', '`Modified`', CastDateFieldForLike("`Modified`", 0, "DB"), 135, 19, 0, false, '`Modified`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->Image_ID->setDbValue($conn->lastInsertId());
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Image_ID', $rs)) {
                AddFilter($where, QuotedName('Image_ID', $this->Dbid) . '=' . QuotedValue($rs['Image_ID'], $this->Image_ID->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Value->Upload->DbValue = $row['Value'];
    ...
    public function deleteUploadedFiles($row)
    {
        ...
        foreach ($oldFiles as $oldFile) {
            if (file_exists($this->Value->oldPhysicalUploadPath() . $oldFile)) {
                @unlink($this->Value->oldPhysicalUploadPath() . $oldFile);
            }
        }
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->Image_ID->CurrentValue : $this->Image_ID->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Image_ID->CurrentValue = $keys[0];
            } else {
                $this->Image_ID->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Image_ID', $row) ? $row['Image_ID'] : null;
        } else {
            $val = $this->Image_ID->OldValue !== null ? $this->Image_ID->OldValue : $this->Image_ID->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Image_ID@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ImageList");
    }
    ...
    {
        global $Language;
        if ($pageName == "ImageView") {
            return $Language->phrase("View");
        } elseif ($pageName == "ImageEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ImageAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ImageView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ImageView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "ImageAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "ImageAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("ImageEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ImageAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("ImageDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "Image_ID:" . JsonEncode($this->Image_ID->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Image_ID->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->Image_ID->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("Image_ID") ?? Route("Image_ID")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->Value->Upload->DbValue = $row['Value'];
        $this->Value->setDbValue($this->Value->Upload->DbValue);
    ...
    public function renderListRow()
    {
        ...
        // Image_ID
        $this->Image_ID->CellCssStyle = "white-space: nowrap;";
        ...
        if (!EmptyValue($this->Value->Upload->DbValue)) {
            ...
            if ($this->isExport()) {
                $this->Value->HrefValue = FullUrl($this->Value->HrefValue, "href");
            }
        } else {
            $this->Value->HrefValue = "";
        }
        $this->Value->ExportHrefValue = $this->Value->UploadPath . $this->Value->Upload->DbValue;
        $this->Value->TooltipValue = "";
        if ($this->Value->UseColorbox) {
            if (EmptyValue($this->Value->TooltipValue)) {
                $this->Value->LinkAttrs["title"] = $Language->phrase("ViewImageGallery");
            }
            $this->Value->LinkAttrs["data-rel"] = "image_x_Value";
            $this->Value->LinkAttrs->appendClass("ew-lightbox");
        }
    ...
    public function renderEditRow()
    {
        ...
        if (!EmptyValue($this->Value->CurrentValue)) {
            $this->Value->Upload->FileName = $this->Value->CurrentValue;
        }
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        if (count($ar) == 1) {
            $this->Image_ID->CurrentValue = $ar[0];
        } else {
