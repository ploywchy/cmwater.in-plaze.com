...
loadjs.ready("head", function () {
    ...
    fproductedit.lists._New = <?= $Page->_New->toClientList($Page) ?>;
...
<?php if ($Page->_New->Visible) { // New ?>
    <div id="r__New" class="form-group row">
        <label id="elh_product__New" class="<?= $Page->LeftColumnClass ?>"><?= $Page->_New->caption() ?><?= $Page->_New->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->_New->cellAttributes() ?>>
<span id="el_product__New">
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->_New->isInvalidClass() ?>" data-table="product" data-field="x__New" name="x__New[]" id="x__New_640507" value="1"<?= ConvertToBool($Page->_New->CurrentValue) ? " checked" : "" ?><?= $Page->_New->editAttributes() ?> aria-describedby="x__New_help">
    <label class="custom-control-label" for="x__New_640507"></label>
</div>
...
<?php } ?>
    ...
    <input type="hidden" data-table="product" data-field="x_Product_ID" data-hidden="1" name="x_Product_ID" id="x_Product_ID" value="<?= HtmlEncode($Page->Product_ID->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
