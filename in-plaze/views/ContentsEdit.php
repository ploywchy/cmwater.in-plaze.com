...
loadjs.ready("head", function () {
    ...
    fcontentsedit = currentForm = new ew.Form("fcontentsedit", "edit");
    ...
    // Validate form
    fcontentsedit.validate = function () {
    ...
    // Form_CustomValidate
    fcontentsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcontentsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fcontentsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcontentsedit" id="fcontentsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fcontentsedit", "editor"], function() {
	ew.createEditor("fcontentsedit", "x_Value", 35, 4, <?= $Page->Value->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<?php } ?>
    ...
    <input type="hidden" data-table="contents" data-field="x_Content_ID" data-hidden="1" name="x_Content_ID" id="x_Content_ID" value="<?= HtmlEncode($Page->Content_ID->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
