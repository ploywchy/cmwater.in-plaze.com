<?php
/**
 * PHPMaker 2021 user level settings
 */
namespace PHPMaker2021\inplaze;

// User level info
$USER_LEVELS = [["-2","Anonymous"],
    ["0","Default"]];
// User level priv info
$USER_LEVEL_PRIVS = [["{inplaze}text","-2","0"],
    ["{inplaze}text","0","0"],
    ["{inplaze}content","-2","0"],
    ["{inplaze}content","0","0"],
    ["{inplaze}image","-2","0"],
    ["{inplaze}image","0","0"],
    ["{inplaze}icon","-2","0"],
    ["{inplaze}icon","0","0"],
    ["{inplaze}icon_list","-2","0"],
    ["{inplaze}icon_list","0","0"],
    ["{inplaze}user","-2","0"],
    ["{inplaze}user","0","0"],
    ["{inplaze}user_level","-2","0"],
    ["{inplaze}user_level","0","0"],
    ["{inplaze}inc_init.php","-2","0"],
    ["{inplaze}inc_init.php","0","0"],
    ["{inplaze}inc_js.php","-2","0"],
    ["{inplaze}inc_js.php","0","0"],
    ["{inplaze}page.php","-2","72"],
    ["{inplaze}page.php","0","0"],
    ["{inplaze}inc_final.php","-2","0"],
    ["{inplaze}inc_final.php","0","0"],
    ["{inplaze}user_level_permission","-2","0"],
    ["{inplaze}user_level_permission","0","0"],
    ["{inplaze}category","-2","0"],
    ["{inplaze}category","0","0"],
    ["{inplaze}product","-2","0"],
    ["{inplaze}product","0","0"],
    ["{inplaze}blog","-2","0"],
    ["{inplaze}blog","0","0"]];
// User level table info
$USER_LEVEL_TABLES = [["text","text","Text",true,"{inplaze}","TextList"],
    ["content","content2","Content",true,"{inplaze}","Content2List"],
    ["image","image","Image",true,"{inplaze}","ImageList"],
    ["icon","icon","Icon",true,"{inplaze}",""],
    ["icon_list","icon_list","Icon Library",true,"{inplaze}",""],
    ["user","user","User",true,"{inplaze}","UserList"],
    ["user_level","user_level2","Level",true,"{inplaze}","UserLevel2List"],
    ["inc_init.php","inc_init","inc_init.php",false,"{inplaze}","IncInit"],
    ["inc_js.php","inc_js","inc_js.php",false,"{inplaze}","IncJs"],
    ["page.php","page2","Webpage",true,"{inplaze}","Page2"],
    ["inc_final.php","inc_final","inc_final.php",false,"{inplaze}","IncFinal"],
    ["user_level_permission","user_level_permission","Permission",true,"{inplaze}",""],
    ["category","category","Category",true,"{inplaze}","CategoryList"],
    ["product","product","Product",true,"{inplaze}","ProductList"],
    ["blog","blog","Blog",true,"{inplaze}","BlogList"]];
